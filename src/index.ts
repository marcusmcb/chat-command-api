...
app.get('/count', (req, res) => {
	...
	if (!value) {
		res
			.status(400)
			.type('text/plain')
			.send('Missing duration. Try /count?duration=10 or /count?duration=2m')
		return
	}
	...
	if (seconds === null) {
		res
			...
			.type('text/plain')
			.send(
				'Invalid duration. Use a number optionally with s/m/h, e.g., 10, 30s, 2m, 1h',
			)
		return
	}
	...
	console.log(
		'Countdown requested with duration:',
		value,
		'| seconds:',
		seconds,
		'| capped:',
	...
	if (
		(process.env.TWITCH_CHANNELS || process.env.TWITCH_CHANNEL) &&
